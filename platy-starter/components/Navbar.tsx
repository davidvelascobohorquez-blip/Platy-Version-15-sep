...
export default function Navbar({ lang: initialLang = "es" as Lang }) {
  ...
  return (
    <header className={`sticky top-0 z-40 backdrop-blur ${scrolled ? "border-b border-[var(--line)] bg-white/80" : "bg-white/50"}`}>
      <div className="container py-3 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2">
          <img src="/brand/PLATY_logo_icon_1024.png" alt="platy" className="h-8 w-8 rounded-lg"/>
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm text-[var(--muted)]">
          ...
          <Link href="/demo" className="btn btn-primary">{t(lang,"nav.demo")}</Link>
